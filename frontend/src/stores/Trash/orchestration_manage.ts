...
import type { ManageAction } from '@/types/actions';
...
const getActionId = (action: ManageAction): string | null => {
    if ('id' in action.params) return action.params.id;
    if ('source_id' in action.params) return (action.params as any).source_id;
    if (action.type === 'manage_create' && action.params.target_id) {
...
export const useOrchestrationManageStore = defineStore('orchestration_manage', () => {
    ...
    const executablePlan = computed((): ManageAction[] => {
    ...
    const dispatchAction = (action: ManageAction) => {
        ...
        historyStore.dispatch(action);

        return entry.id; 
    };

    const updateActionParams = (entryId: string, newParams: Partial<ManageAction['params']>) => {
    ...
    const undoAction = (actionToUndo: ManageAction) => {
    ...
    const getFinalActionForIdByTypes = (itemId: string, types: ManageAction['type'][], options?: { includeCancelled: boolean }) => {
        const includeCancelled = options?.includeCancelled || false;
        return [...log.value].reverse().find(e => 
            (includeCancelled || !e.isCancelled) &&
            getActionId(e.action) === itemId &&
    ...
    const isActionQueued = (itemId: string, actionType: ManageAction['type']) => {
